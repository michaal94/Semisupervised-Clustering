...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', default='MNIST-train', choices=['MNIST-train', 'custom', 'MNIST-test', 'MNIST-full'],
    ...
    parser.add_argument('--leaky', default=True, type=str2bool, help='use leaky version of relu')
    parser.add_argument('--neg_slope', default=0.01, type=float, help='negative slope for leaky relu')
    parser.add_argument('--activations', default=False, type=str2bool, help='use sigmoid and tanh activations in autoencoder')
    parser.add_argument('--bias', default=True, type=str2bool, help='use bias in layers')
    ...
    if dataset == 'MNIST-train':
        ...
        dataset_labelled = mnist.MNIST('../data', train=True, download=True, small=True,
                                 transform=transforms.Compose([
                                     transforms.ToTensor(),
                                     # transforms.Normalize((0.1307,), (0.3081,))
                                 ]))
        ...
        dataset_labelled = mnist.MNIST('../data', train=False, download=True, small=True,
                                       transform=transforms.Compose([
                                           transforms.ToTensor(),
                                           # transforms.Normalize((0.1307,), (0.3081,))
                                       ]))
        ...
        dataset_labelled = mnist.MNIST('../data', full=True, download=True, small=True,
                                       transform=transforms.Compose([
                                           transforms.ToTensor(),
                                           # transforms.Normalize((0.1307,), (0.3081,))
                                       ]))
